//
//  HYBProgressHUD.swift
//  移动医生
//
//  Created by 杨生青 on 16/4/13.
...
class YSQProgressHUD: UIView {
    ...
    ///
    /// @brief 单例方法，只允许内部调用
    private class func sharedInstance() -> YSQProgressHUD {
        struct Instance {
            static var onceToken: dispatch_once_t = 0
            static var instance: YSQProgressHUD?
        }
        
        dispatch_once(&Instance.onceToken, { () -> Void in
            Instance.instance = YSQProgressHUD(frame: UIScreen.mainScreen().bounds)
            Instance.instance?.setStyle(YSQProgressHUDStyle.WhiteHUDStyle)
        })
    ...
    private func configureHUD(status: String?, image: UIImage?, isSpin: Bool, isHide: Bool) {
        ...
        if isHide {
            NSThread.detachNewThreadSelector(#selector(YSQProgressHUD.hideWhenTimeout), toTarget: self, withObject: nil)
        }
    }
    
    ///
    /// @brief 设置风格样式，默认使用的是黑色的风格，如果需要改成白色的风格，请在内部修改样式
    private func setStyle(style: YSQProgressHUDStyle) {
        switch style {
        case .BlackHUDStyle:
    ...
    private func configureProgressHUD() {
        if hud == nil {
            ...
            /// 监听设置方向变化
            NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(YSQProgressHUD.rotate(_:)), name: UIDeviceOrientationDidChangeNotification, object: nil)
        }
    ...
    func hideWhenTimeout() {  
        autoreleasepool { () -> () in  
            let length = countElements(self.titleLabel!.text!)  
            let sleepTime: NSTimeInterval = NSTimeInterval(length) * 0.04 + 0.5  
    ...
    func countElements(text:NSString) -> Int {
        return text.length;
    }
}
